...
test_that("getLineages works for different input types", {
    ...
    expect_true(all(c(
        "start.clus", "end.clus", "start.given", "end.given",
        "omega", "omega_scale"
    ) %in% names(slingParams(c0))))
    ...
    clmat <- sapply(unique(cl), function(cln) {
        as.numeric(cl == cln)
    })
    ...
    # invalid inputs
    expect_error(
        getLineages(sce, reducedDim = "UMAP"),
        "not found in reducedDims"
    )
    expect_error(
        getLineages(sce, clusterLabels = "pam"),
        "not found in colData"
    )


    # invalid inputs
    expect_error(getLineages(
        reducedDim[, -(seq_len(ncol(reducedDim)))],
        clusterLabels
    ), "has zero columns")
    expect_error(getLineages(
        reducedDim[-(seq_len(nrow(reducedDim))), ],
        character(0)
    ), "has zero rows")
    expect_error(
        getLineages(reducedDim, clusterLabels[seq_len(10)]),
        "must equal"
    )
    rdna <- reducedDim
    rdna[1, 1] <- NA
    expect_error(
        getLineages(rdna, clusterLabels),
        "cannot contain missing values"
    )
    rdc <- reducedDim
    rdc[1, 1] <- "a"
    expect_error(
        getLineages(rdc, clusterLabels),
        "must only contain numeric values"
    )
    expect_error(getLineages(reducedDim, cl.imb[-1, ]), "must equal nrow")
    expect_error(
        getLineages(reducedDim, clusterLabels, omega = 1:5),
        "omega must have length 1"
    )
    expect_error(
        getLineages(reducedDim, clusterLabels, omega = NA),
        "omega must be logical or numeric"
    )
    expect_error(
        getLineages(reducedDim, clusterLabels, omega = -1),
        "omega must be non-negative"
    )

    expect_error(
        getLineages(reducedDim, NULL),
...
test_that("getLineages works as expected", {
    ...
    expect_true(any(vapply(slingLineages(sds2), function(l) {
        (l[1] == "1") &&
            (l[length(l)] == "3")
    }, TRUE)))
    ...
    expect_true(all(as.character(4:5) %in% c(
        slingLineages(sdsO)$Lineage2,
...
test_that("slingshot works for different input types", {
    ...
    rdna[1, 1] <- NA
    expect_error(
        slingshot(rdna, clusterLabels),
        "cannot contain missing values"
    )
    rdc <- reducedDim
    rdc[1, 1] <- "a"
    expect_error(
        slingshot(rdc, clusterLabels),
        "must only contain numeric values"
    )
    expect_error(
        slingshot(rd, NULL),
    ...
    expect_error(
        slingshot(sce, clusterLabels = "pam"),
        "not found in colData"
    )
})
...
test_that("slingshot works with ClusterExperiment objects", {
    ...
    # invalid inputs
    expect_error(
        slingshot(ce, reducedDim = "UMAP"),
        "not found in reducedDims"
    )
    expect_error(
        slingshot(ce, clusterLabels = "pam"),
        "not found in colData"
    )
})
...
test_that("3D plotting functions work as expected", {
    ...
    expect_error(
        plot3d.SlingshotDataSet(sds, linInd = 3:5),
        "None of the provided lineage indices"
    )
    ...
    # errors
    expect_error(
        plot3d.SlingshotDataSet(sds0),
        "No lineages or curves detected"
    )
    expect_error(
        plot3d.SlingshotDataSet(sds, type = "foo"),
        "Unrecognized type argument"
    )
    expect_error(
        plot3d.SlingshotDataSet(sds0, type = "lineages"),
        "No lineages detected"
    )
    expect_error(
        plot3d.SlingshotDataSet(sds1, type = "curves"),
        "No curves detected"
    )
})
...
test_that("Helper functions work as expected", {
    ...
    expect_equal(
        ncol(slingMST(pto, as.df = TRUE)),
        ncol(slingReducedDim(pto)) + 3
    )
    expect_equal(
        sort(unique(slingMST(pto, as.df = TRUE)$Cluster)),
        as.character(1:5)
    )
    ...
    expect_equal(
        ncol(slingCurves(pto, as.df = TRUE)),
    ...
    expect_message(
        {
            df0 <- newSlingshotDataSet(data.frame(rd))
        },
        "Unclustered data detected."
    )
