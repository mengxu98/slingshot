...
setMethod(
    ...
    definition = function(object) {
        ...
        df <- data.frame(
            Samples = nrow(reducedDim(object)),
...
.avg_curves <- function(pcurves, X, stretch = 2, approx_points = FALSE) {
    n <- nrow(pcurves[[1]]$s)
    p <- ncol(pcurves[[1]]$s)
    max.shared.lambda <- min(vapply(pcurves, function(pcv) {
        max(pcv$lambda)
    }, 0))
    ...
    pcurves.dense <- lapply(pcurves, function(pcv) {
        vapply(seq_len(p), function(jj) {
            ...
            interpolated <- approx(xin_lambda[pcv$ord],
                ...
                xout = lambdas.combine, ties = "ordered"
            )$y
            return(interpolated)
        }, rep(0, n))
    })
    ...
    if (approx_points > 0) {
        ...
        avg.curve$s <- apply(avg.curve$s, 2, function(sjj) {
            return(approx(
                x = avg.curve$lambda[avg.curve$ord],
                y = sjj[avg.curve$ord],
                xout = xout_lambda, ties = "ordered"
            )$y)
        })
        avg.curve$ord <- seq_len(approx_points)
    }

    avg.curve$w <- rowSums(vapply(pcurves, function(p) {
        p$w
    }, rep(0, nrow(X))))
    return(avg.curve)
}
...
.percent_shrinkage <- function(crv, share.idx, approx_points = FALSE,
    ...
    if (method %in% eval(formals(density.default)$kernel)) {
        ...
        if (box.vals[1] == box.vals[5]) {
            ...
            pct.l <- approx(surv$x, surv$y, pts2wt,
                rule = 2,
                ties = "ordered"
            )$y
        }
    ...
    if (method == "tricube") {
        tc <- function(x) {
            ifelse(abs(x) <= 1, (70 / 81) * ((1 - abs(x)^3)^3), 0)
        }
    ...
    if (method == "density") {
        ...
        pct.l <- (approx(d2$x, d2$y,
            xout = pts2wt, yleft = 0,
            yright = 0, ties = mean
        )$y * scale) /
            approx(d1$x, d1$y,
                xout = pts2wt, yleft = 0, yright = 0,
                ties = mean
            )$y
        pct.l[is.na(pct.l)] <- 0
...
.shrink_to_avg <- function(pcurve, avg.curve, pct, X, approx_points = FALSE,
    ...
    s <- vapply(seq_len(p), function(jj) {
        orig.jj <- pcurve$s[, jj]
        avg.jj <- approx(
            x = avlam, y = avg.curve$s[, jj], xout = lam,
            rule = 2, ties = mean
        )$y
