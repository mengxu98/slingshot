...
setMethod(
    ...
    definition = function(x, newDimRed,
        ...
        if (shrink < 0 | shrink > 1) {
            stop(
                "'shrink' parameter must be logical or numeric between",
                " 0 and 1"
            )
        }
        if (nrow(X) != nrow(newX)) {
            stop(
                "'newX' must have same number of rows as original",
                "'reducedDim'."
            )
        }
        if (any(is.na(newX))) {
        ...
        for (l in seq_len(L)) {
            ...
            for (jj in seq_len(p.new)) {
                yjj <- smootherFcn(pcurve$lambda, newX[, jj],
                    w = pcurve$w,
                    ...
                )[ordL]
                if (approx_points > 0) {
                    yjj <- approx(
                        x = pcurve$lambda[ordL], y = yjj,
            ...
            if (approx_points > 0) {
                ...
                new.pcurve$s <- apply(new.pcurve$s, 2, function(sjj) {
                    return(approx(
                        x = new.pcurve$lambda[new.pcurve$ord],
